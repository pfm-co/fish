/**
 * Created by Arman on 2016-11-03.
 *
 * @providesModule FmHeader
 * @flow
 */

'use strict';

var FmColors = require('./FmColors');
...
class FmHeaderAndroid extends React.Component {
...
class FmHeaderIOS extends React.Component {
    static height: number;
    props: Props;

    render() {
        const {leftItem, title, rightItem, foreground} = this.props;
        const titleColor = foreground === 'dark' ? FmColors.darkText : 'white';
        const itemsColor = foreground === 'dark' ? FmColors.lightText : 'white';
...
const Header = Platform.OS === 'ios' ? FmHeaderIOS : FmHeaderAndroid;
